...
object ApplicationUtils {
    ...
    fun putRequest(url: String, headers: List<Pair<String, String>>, body: String): String {
        val client = HttpClient.newBuilder().build()
        val request = httpRequestBuilder(url=url, headers=headers)
        ...
        val response = client.sendAsync(request, HttpResponse.BodyHandlers.ofString()).get()
        logCRUD(request, response, body)
        checkPutResponseStatus(response.statusCode())
        return response.body()
    }
    ...
    private fun checkPutResponseStatus(statusCode: Int) {
        when (statusCode) {
            200, 204 -> { // 200 OK or 204 No Content
                log.info("PUT OK")
            }
            404 -> {
                throw NoSuchElementException("Element not found: $statusCode")
